...
@router.delete("/me")
async def delete_me(
    user: User = Depends(get_current_user), repo: Repository = Depends(get_repository)
...
async def handle_user_login(repo: Repository, scoped_id: str) -> RedirectResponse:
...
async def orcid_callback(request: Request, repo: Repository = Depends(get_repository)) -> RedirectResponse:
...
async def github_callback(request: Request, repo: Repository = Depends(get_repository)) -> RedirectResponse:
